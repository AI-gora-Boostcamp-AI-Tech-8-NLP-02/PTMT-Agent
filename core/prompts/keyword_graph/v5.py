...
KEYWORD_GRAPH_PROMPT_V5 = ChatPromptTemplate.from_messages([
    ("system", """
...
4. Semantic Duplicate and Paper-like Term Removal
- Remove only clear and unambiguous duplicates (exact synonyms or naming variants)
- Remove keywords that directly refer to the target paper itself or are nearly identical to the paper title
  (e.g., paper titles or trivial naming variants of the paper)
...
7. Target Paper Connection (Required)
- The final graph MUST include at least one edge that connects to the target paper via IN or ABOUT,
...
8. Graph Connectivity and Consistency
